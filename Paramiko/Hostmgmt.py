...
#Configure the hostname to each host
def write_etc_host():
    exit_flag = False
    cmd_list = []
            ...
            if len(err_list)>0:
                ...
                for out in stdout.readlines():
                    if 'Error' in out or 'ERROR' in out:
                        ...
                        sys.exit('\033[1;31m Some node unreachable, please check the node service status\033[0m')
                    else:
                        print out.rstrip()
        ssh.close()
        return True
...
#Remote connect to the hosts and check cluster components installation status
...
#Download the rpm packages from ftp server.
#The ftp link is like this: ftp://192.168.100.97/orcafs-packages/OrcaFS-17957644/
#The required input format: OrcaFS-17957644/
def rpm_Download():
    #exit_flag=False
    cmd_list=[]
...
#Transfer scripts to remote host
...
#Remote install the orcafs rpm packages
def remote_install():
...
#If have multiple Storage Server, loop to input the RAID name, RAID type, Disk count and Mount Path.
#RAID name: <string>
#RAID type: 5 or 6
#Format type: ext4 or xfs
#Disk count: if type is 5, the minimum count is 3; if type is 6, the minimum count is 4.
#Mount Path: <string>, format is like: mnt/md0, xxx/xxxx
...
#If have multiple Metadata Server, loop to input the RAID name, RAID type, Disk count and Mount Path.
#RAID name: <string>
#RAID type: 1 or 10
#Format type: ext4 or xfs
#Disk count: if type is 1, the minimum count is 2; if type is 10, the minimum count is 4.
#Mount Path: <string>, format is like: mnt/md0, xxx/xxxx
...
#Check the orcafs components service status and start the service
