...
tree = ET.parse("/Users/hongrui/PycharmProjects/Orcadt/Python-Learning/XML-Excel/Backup.testproject-deep.xml")
# 获取 xml 根目录
root = tree.getroot()
# 获取 测试用例集
...
for num_suite in range(0, len(testsuite_per)):
    ...
    for testsuite_son in testsuite_per[num_suite].findall('testsuite'):
        ...
        #while True:
        if testsuite_son.findall('testsuite') is not None:
            for son_son in testsuite_son.findall('testsuite'):
                if son_son.findall('testsuite') is not None:
                    for son_son_son in son_son.findall('testsuite'):
                        testcase = son_son_son.getiterator('testcase')
                        num = len(testcase)
        # else:
